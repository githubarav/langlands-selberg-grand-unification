...
-/
theorem prove_maass_functoriality
  (Nbh : Neighborhood)
  (Hecke_Eigenvalue : AutomorphicPoint → ℝ)
  -- Requirement 1: The Hecke function is continuous
  (h_cont : ∀ (seq : ℕ → AutomorphicPoint) (target : AutomorphicPoint),
  ...
  have ⟨seq, h_seq_holo, h_seq_converges⟩ :=
  eigenvariety_density
    Nbh
    nesting_property
    h_dense
    m
...
abbrev AutomorphicPoint := Padic_Rigid_Geometry.AutomorphicPoint
abbrev FormType := Padic_Rigid_Geometry.FormType

theorem prove_maass_functoriality_rebased
  (Nbh : Padic_Rigid_Geometry.Neighborhood)
  (Hecke_Eigenvalue : AutomorphicPoint → ℝ)
  (h_cont : ∀ (seq : ℕ → AutomorphicPoint) (target : AutomorphicPoint),
    Padic_Rigid_Geometry.converges_to Nbh seq target →
    ∀ ε > 0, ∃ N : ℕ, ∀ n ≥ N,
      |Hecke_Eigenvalue (seq n) - Hecke_Eigenvalue target| < ε)
  (h_link : ∀ p : AutomorphicPoint, Hecke_Eigenvalue p = 0 ↔ p.satake = 0)
  (m : AutomorphicPoint)
  (seq : ℕ → AutomorphicPoint)
  (h_seq_zeros : ∀ n, (seq n).satake = 0)
  (nesting_property : ∀ k n p target, n ≥ k → Nbh n p target → Nbh k p target)
  (h_dense : Padic_Rigid_Geometry.space_is_dense Nbh)
  (h_maass : m.type = Padic_Rigid_Geometry.FormType.Maass) :
  m.satake = 0 := by
  simpa using
    (Padic_Rigid_Geometry.prove_maass_functoriality
      Nbh
      Hecke_Eigenvalue
      h_cont
      h_link
